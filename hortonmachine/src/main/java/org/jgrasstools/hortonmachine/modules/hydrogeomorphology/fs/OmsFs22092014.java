...
public class OmsFs22092014 extends JGTModel {
	...
	public void process() throws Exception {
		...
		String fileee = listOfFiles[0].getName().toString();
		String[] indiceTime = fileee.split("N")[1].split(".asc");
		String indiceTimee = indiceTime[0];
		...
		if (listOfFiles.length < nLayers) {
			...
			for (int i = 0; i < nLayers; i++) {
				...
				String pathToTheta = pathToGEOtopOutputMaps + "/"
						+ headerThetaGEOtop + "L" + indiceLayer + "N"
						+ indicetime + ".asc";
	...
	public double computeFSvalue(double slopeValue, double phiValue,
		...
		double fsModified = Math.min(fs, 100);
