...
public class OmsComputeFS_GSE_PROBABILISTIC extends JGTModel {
	...
	@Description(OMSSHALSTAB_inSlope_DESCRIPTION)
	@In
	public boolean doProbabilistic = false;

	@Description(OMSSHALSTAB_inSlope_DESCRIPTION)
	@In
	public double[] inParVectMin = null;

	@Description(OMSSHALSTAB_inSlope_DESCRIPTION)
	@In
	public double[] inParVectMax = null;
	...
	private void qcrit(RenderedImage slope, RenderedImage ab,
		...
		double temporary_matrix[][] = new double[rows][cols];
		for (int iter = 0; iter < 1000; iter++) {
			for (int j = 0; j < rows; j++) {
				...
				for (int i = 0; i < cols; i++) {
					...
					// no units
					if (doProbabilistic) {
						tangPhiValue = UNIFORM(inParVectMin[0],
								inParVectMax[0], 1)[0];
					}
					...
					if (doProbabilistic) {
						cohValue = UNIFORM(inParVectMin[1], inParVectMax[1], 1)[0];
					}
					double gsValue = gsIter.getSampleDouble(i, j, 0);
					// no unit
					if (doProbabilistic) {
						gsValue = UNIFORM(inParVectMin[2], inParVectMax[2], 1)[0];
					}
					double srValue = degreeOfSatIter.getSampleDouble(i, j, 0);
					if (doProbabilistic) {
						srValue = UNIFORM(inParVectMin[3], inParVectMax[3], 1)[0];
					}
					// no unit

					double hsValue = hsIter.getSampleDouble(i, j, 0);
					// m
					double soilPorosityValue = soilPorosityIter
							.getSampleDouble(i, j, 0);
					if (doProbabilistic) {
						soilPorosityValue = UNIFORM(inParVectMin[4],
								inParVectMax[4], 1)[0];
					}
					// no unit
					double rainfall_m_giorno = effectiveRI.getSampleDouble(i,
							j, 0) / 1000;
					if (doProbabilistic) {
						rainfall_m_giorno = UNIFORM(inParVectMin[5],
								inParVectMax[5], 1)[0] / 1000;
					}
					// m/g
					double rhoW = rhoWIter.getSampleDouble(i, j, 0);
					// kg/m3
					double transmissivity_m2_g = trasmissivityRI
							.getSampleDouble(i, j, 0);
					if (doProbabilistic) {
						transmissivity_m2_g = UNIFORM(inParVectMin[5],
								inParVectMax[5], 1)[0] / 1000;
					}
					// m2/g
					if (computeTransmissivity) {
						transmissivity_m2_g = trasmissivityRI.getSampleDouble(
								i, j, 0) * hsValue;
						// m2/g
					}
					if (!isNovalue(slopeValue) && !isNovalue(abValue)
							&& !isNovalue(tangPhiValue) && !isNovalue(cohValue)
							&& !isNovalue(soilPorosityValue)
							&& !isNovalue(hsValue) && !isNovalue(srValue)
							&& !isNovalue(rhoW) && !isNovalue(gsValue)
						...
						if ((hsValue <= EPS || slopeValue > pRock)
							...
							if (pMode == 0) {
								...
								if (pMode == 1) {
									...
									System.out
											.println("Error in setting pMode");
							...
							double fsnumPhi = ((gsValue + eValue * srValue - w
									* (1. + eValue * srValue)) / (gsValue
									+ eValue * srValue + w * eValue
									* (1.0 - srValue)))
									* tangPhiValue / slopeValue;
							double fs = fsnumC + fsnumPhi;
							if (fs >= pThresholdFS) {
								//temporary_matrix[j][i] = classiIter.getSample(
								//		i, j, 0);
								//classiIter.setSample(i, j, 0, 0);

							} else {
								temporary_matrix[j][i] = temporary_matrix[j][i] + 1;
								classiIter.setSample(i, j, 0,
										temporary_matrix[j][i]);
